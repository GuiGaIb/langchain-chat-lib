...
const outputSchema = z.object({
  document_names: z
    .array(z.string().trim())
    .describe(
      'The names of the documents that might be relevant to the chat. This property is required, but it can be an empty array.'
    ),
});
...
const execKnowledgeQuery = RunnableLambda.from<
  LCRunOutput<typeof getKnowledgeQuery>,
  string[]
>((input) =>
  ServiceIndexDAO.Knowledge.find()
    .byNames(input.document_names)
    .exec()
    .then((docs) => docs.map((doc) => doc.stringify()))
).withConfig({ runName: 'exec_knowledge_query' });
